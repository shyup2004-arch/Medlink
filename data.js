const latestLectureData = {
    ...
    "professor": "김재봉",
    "summary": "세포의 에너지 대사를 이해하는 핵심 챕터입니다. 에너지를 어떻게 얻고(이화작용), 어떻게 사용하는지(동화작용)를 ATP라는 화폐를 중심으로 배웁니다. 김재봉 교수님은 **각 과정의 특정 중간산물**을 묻는 단답형을 선호하시니, 큰 흐름과 함께 핵심 분자 이름을 암기하는 것이 중요합니다.<br><h4>🔑 핵심 암기 사항</h4><ul><li><strong>자유 에너지(Gibbs Free Energy, ΔG):</strong> ΔG가 음수(-)일 때 자발적 반응이라는 개념을 이해해야 합니다.</li><li><strong>ATP (Adenosine Triphosphate):</strong> 세포의 에너지 화폐. ATP가 ADP로 가수분해될 때 방출되는 에너지를 통해 비자발적 반응을 진행시킵니다.</li><li><strong>주요 대사 경로:</strong> 해당과정(Glycolysis) → TCA 회로(Krebs Cycle) → 산화적 인산화(Oxidative Phosphorylation)로 이어지는 큰 흐름을 잡아야 합니다.</li></ul><h4><span class=\"text-red\">❗️ 기출 연관 중요 개념</span></h4><ul><li><strong>TCA 회로의 첫 산물:</strong> <span class=\"text-red\">2023년 1차 시험</span>에서 **옥살아세트산(Oxaloacetate)과 아세틸 CoA가 만나 생성되는 첫 6탄당**을 묻는 문제가 출제되었습니다. 정답은 <strong>시트르산(Citrate)</strong>입니다.</li><li><strong>해당과정의 3탄당:</strong> 포도당이 2개의 3탄당으로 쪼개진 후의 산물인 <strong>Glyceraldehyde-3-phosphate (G3P)</strong>와 <strong>Dihydroxyacetone phosphate (DHAP)</strong>를 기억해야 합니다.</li><li><strong>전자전달계 복합체:</strong> <span class=\"text-red\">(2023년 1차 시험 출제)</span> 전자를 전달하며 양성자(H+)를 펌핑하는 **Complex I, III, IV**를 알아야 합니다.</li></ul><h4>🧪 주요 용어</h4><ul><li><strong>기질 수준 인산화(Substrate-level phosphorylation):</strong> 해당과정과 TCA 회로에서 일어나는 소량의 ATP 생성 방식.</li><li><strong>화학삼투(Chemiosmosis):</strong> 산화적 인산화에서 양성자 기울기를 이용해 ATP를 합성하는 원리.</li></ul>",
    "reviewQuiz": [
        ...
        { "question": "산화적 인산화 과정에서, 전자전달계의 최종 전자 수용체는 어떤 분자인가요?", "source": "AI 예상 문제", "answer": "산소 (O2)" }
    ]
};
...
const lectureData = [
    ...
    { 
        ...
        "professor": "김재봉", 
        "summary": "세포의 에너지 대사를 이해하는 핵심 챕터입니다. 에너지를 어떻게 얻고(이화작용), 어떻게 사용하는지(동화작용)를 ATP라는 화폐를 중심으로 배웁니다. 김재봉 교수님은 <strong>각 과정의 특정 중간산물</strong>을 묻는 단답형을 선호하시니, 큰 흐름과 함께 핵심 분자 이름을 암기하는 것이 중요합니다.<br><h4>🔑 핵심 암기 사항</h4><ul><li><strong>자유 에너지(Gibbs Free Energy, ΔG):** ΔG가 음수(-)일 때 자발적 반응이라는 개념을 이해해야 합니다.</li><li><strong>ATP (Adenosine Triphosphate):** 세포의 에너지 화폐. ATP가 ADP로 가수분해될 때 방출되는 에너지를 통해 비자발적 반응을 진행시킵니다.</li><li><strong>주요 대사 경로:</strong> 해당과정(Glycolysis) → TCA 회로(Krebs Cycle) → 산화적 인산화(Oxidative Phosphorylation)로 이어지는 큰 흐름을 잡아야 합니다.</li></ul><h4><span class=\"text-red\">❗️ 기출 연관 중요 개념</span></h4><ul><li><strong>TCA 회로의 첫 산물:</strong> <span class=\"text-red\">2023년 1차 시험</span>에서 **옥살아세트산(Oxaloacetate)과 아세틸 CoA가 만나 생성되는 첫 6탄당**을 묻는 문제가 출제되었습니다. 정답은 <strong>시트르산(Citrate)</strong>입니다.</li><li><strong>해당과정의 3탄당:</strong> 포도당이 2개의 3탄당으로 쪼개진 후의 산물인 <strong>Glyceraldehyde-3-phosphate (G3P)</strong>와 <strong>Dihydroxyacetone phosphate (DHAP)</strong>를 기억해야 합니다.</li><li><strong>전자전달계 복합체:** <span class=\"text-red\">(2023년 1차 시험 출제)</span> 전자를 전달하며 양성자(H+)를 펌핑하는 **Complex I, III, IV**를 알아야 합니다.</li></ul><h4>🧪 주요 용어</h4><ul><li><strong>기질 수준 인산화(Substrate-level phosphorylation):** 해당과정과 TCA 회로에서 일어나는 소량의 ATP 생성 방식.</li><li><strong>화학삼투(Chemiosmosis):** 산화적 인산화에서 양성자 기울기를 이용해 ATP를 합성하는 원리.</li></ul>"
    },
...
const questionData = [
    ...
    { "id": 902, "chapter": [17], "professor": "김재봉", "exam": "4차", "question": "Ras/Raf/MEK/ERK pathway의 구성 요소를 순서대로 나열하시오. (핵 내부 전사인자 포함)", "answer": "Ras -> Raf -> ME.K -> ERK -> Elk-1" },
