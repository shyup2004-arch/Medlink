const latestLectureData = {
    "title": "Chapter 1: An Overview of Cells and Cell Research (2025)",
    "professor": "김성찬",
    "summary": `🗓️ **9월 1일 (Update): 세포와 세포 연구 개요 (Ch.1) 개념 총정리**<br>
    새로운 강의 자료에 맞춰 업데이트된, 오늘 수업 예습을 위한 상세 요약본입니다.
    <h4>1️⃣ 생명의 기원과 세포의 화학적 기초</h4>
    <ul>
        <li><strong>세포의 발견 🧐:</strong> 1665년 <strong>로버트 훅(Robert Hooke)</strong>이 코르크에서 처음 발견했으며, 이후 안톤 판 레이우엔훅(Antonie van Leeuwenhoek)이 살아있는 세포를 관찰했습니다.</li>
        <li><strong>세포 이론 (Cell Theory) 📜:</strong> 슐라이덴과 슈반이 정립한 이론으로, ①모든 생물은 세포로 구성 ②세포는 생명의 기본 단위 ③세포는 기존 세포로부터만 생성된다는 내용을 담고 있습니다.</li>
        <li><strong>생명의 화학적 기원:</strong> 1953년 <strong>밀러(Miller)와 유레이(Urey)의 실험</strong>을 통해 원시 지구 환경에서 무기물로부터 유기 분자(Organic molecules)가 자발적으로 생성될 수 있음이 증명되었습니다.</li>
        <li><strong>RNA 세계 가설 (RNA World Hypothesis) 🧬:</strong> 초기 생명체에서는 <strong>RNA</strong>가 유전 정보 저장(DNA처럼)과 화학 반응 촉매(단백질 효소처럼) 역할을 모두 수행했을 것이라는 핵심 가설입니다.</li>
        <li><strong>인지질 (Phospholipids):</strong> 세포막의 기본 구성 요소로, 친수성 머리와 소수성 꼬리를 가진 <strong>양친매성(Amphipathic)</strong> 분자입니다. 이 성질 때문에 물에서 자발적으로 이중층(Bilayer)을 형성하여 세포의 경계를 만듭니다.</li>
        <li><strong>ATP (Adenosine Triphosphate) ⚡️:</strong> 모든 세포 활동의 직접적인 에너지원으로 사용되는 '에너지 화폐'입니다.</li>
    </ul>
    <h4>2️⃣ 세포의 진화와 다양성</h4>
    <ul>
        <li><strong>대사 과정의 발달:</strong> <strong>해당과정 (Glycolysis)</strong> → <strong>광합성 (Photosynthesis)</strong> → <strong>산화적 호흡 (Oxidative respiration)</strong> 순서로 발달했습니다.</li>
        <li><strong>세포의 종류:</strong> 핵막과 세포 소기관이 없는 <strong>원핵세포 (Prokaryotes)</strong>와, 막으로 둘러싸인 핵(Nucleus)과 다양한 세포 소기관을 가지는 <strong>진핵세포 (Eukaryotes)</strong>로 나뉩니다.</li>
        <li><strong>세포 내 공생설 (Endosymbiosis) 🤝:</strong> 진핵세포가 호기성 세균과 광합성 세균을 삼켜 각각 <strong>미토콘드리아</strong>와 <strong>엽록체</strong>로 만들었다는 이론입니다.</li>
        <li><strong>다세포 생물의 출현:</strong> 세포들이 특정 기능을 갖도록 <strong>분화(Differentiation)</strong>하면서 상피 조직, 결합 조직(뼈, 혈액 포함), 신경 조직, 근육 조직 등을 형성하게 되었습니다.</li>
    </ul>
    <h4>3️⃣ 세포 연구를 위한 모델 시스템</h4>
    <ul>
        <li><strong>대장균 (E. coli):</strong> 분자생물학의 근본 원리를 밝히는 데 사용된 대표적인 원핵 모델입니다.</li>
        <li><strong>효모 (Saccharomyces cerevisiae):</strong> 가장 단순한 진핵세포 모델로, 세포 주기(cell cycle) 연구에 기여했습니다.</li>
        <li><strong>예쁜꼬마선충 (C. elegans):</strong> 발생 과정과 프로그램된 세포 사멸(programmed cell death) 연구에 이상적입니다.</li>
        <li><strong>초파리 (Drosophila melanogaster):</strong> 유전학 연구의 고전적인 모델입니다.</li>
        <li><strong>생쥐 (Mus musculus) 🐁:</strong> 인간과 유전적으로 매우 유사하여 다양한 인간 질병 연구에 필수적인 포유류 모델입니다.</li>
    </ul>
    <h4>4️⃣ 세포 생물학의 주요 연구 도구</h4>
    <ul>
        <li><strong>현미경 (Microscopy) 🔭:</strong>
            <ul>
                <li><strong>해상력 (Resolution):</strong> 두 점을 구분하는 능력으로, 광학 현미경의 한계는 약 0.2 마이크로미터입니다.</li>
                <li><strong>형광 현미경:</strong> <strong>GFP(Green Fluorescent Protein)</strong>를 이용해 살아있는 세포 내 특정 단백질의 위치와 이동을 시각화합니다.</li>
                <li><strong>전자 현미경:</strong> <strong>TEM</strong>은 세포 내부 구조를, <strong>SEM</strong>은 세포 표면의 3차원 구조를 보여줍니다.</li>
            </ul>
        </li>
        <li><strong>세포 배양 (Cell Culture) 🧪:</strong> 동물 조직에서 세포를 분리하여 실험실 환경(in vitro)에서 키우는 기술입니다. (예: HeLa 세포주)</li>
    </ul>`,
    "reviewQuiz": [
        { "question": "A segment of DNA that is expressed to yield a functional product (e.g., a polypeptide or RNA molecule) is called a ____.", "source": "23년 1차 기출", "answer": "Gene (유전자)" },
        { "question": "성인의 체세포를 역분화시켜 배아줄기세포와 유사한 특성을 갖도록 만든 세포는 무엇인가요? (영문 약어)", "source": "23년 1차 기출", "answer": "iPS cells (유도만능줄기세포)" },
        { "question": "초기 생명체에서 유전 정보 저장과 효소 역할을 모두 수행했을 것으로 추정되는 분자는 무엇이며, 이 가설의 이름은 무엇인가요?", "source": "AI 예상 문제", "answer": "분자: RNA, 가설: RNA 세계 가설 (RNA World Hypothesis)" }
    ]
};
...
const aiQuestionData = [
    { "id": "ai_101", "chapter": [1], "professor": "김성찬", "question": "진핵세포가 원핵세포로부터 진화했다고 보는 '세포 내 공생설'의 핵심 내용과, 이를 뒷받침하는 미토콘드리아의 증거 두 가지를 서술하시오.", "answer": "핵심 내용: 과거의 진핵세포가 호기성 세균(aerobic bacteria)을 삼켜 공생 관계를 이루면서, 이 세균이 미토콘드리아로 분화했다는 이론입니다.\n증거:\n1. 미토콘드리아는 독자적인 원형 DNA를 가지고 있습니다.\n2. 미토콘드리아는 내막과 외막의 이중막 구조를 가집니다." },
